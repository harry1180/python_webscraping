...
def extractQueries(sourceFolder="SourceXMLs",targetFolder="Sanity_checks_op",targetFileExt=".txt"):
    ...
    for inputFile in filesList:
        ...
        task_type = soup.find_all("taskinstance", attrs = {"tasktype":"Session"})   # finding the string taskinstance
        for each in task_type:
            code.write(each['name']+":")
            code.write("\n")
                                                                                    # Fail Parent if this task fails & Fail parent if this task does not run property should be checked
            ...
            if each["fail_parent_if_instance_fails"]!= 'YES':                  
                ...
                win32api.MessageBox(0, 'fail_parent_if_instance_fails is ticked', 'title')
                code.write("\n")
        ...
        for instance in sourceContents1:
            ...
            for eachTag in instanceContents1:
                if (eachTag['name']=="Tracing Level" ):
                 code.write("Tracing Level :   "+instance['name'])
                 code.write(":  "+eachTag['value'])
                 code.write("\n")
        code.write("\n")    
        ...
        for instance in configContents:                   # Stop on error property should be 1 and save session logs value should be 5
            tempSrcName=instance['name']
            ...
            for eachTag in instanceContents:
                ...
                if (eachTag['name']== "Pre 85 Timestamp Compatibility" and eachTag['value']!="YES"):     #  Pre time stamp compatibility should be enabled
                ...
                if (eachTag['name']== "Override tracing" and eachTag['value']!="Normal"):                # override tracing should be normal
        ...
        for instance in sessionwContents:     
            ...
            code.write("\n")
            for eachTag in instanceContents:                                                             # checking reject file directory and reject file name format
        ...
        # checking session log file directory , parameter file name , session log file name and pushdown optimisation
